...
const SCORE_HEIGHT = SQUARE_SIZE;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  newGameBtns.forEach(btn => btn.addEventListener('click', () => {
    ...
    allModals.forEach(modal => { modal.className = 'game-modal'; });
    game = new __WEBPACK_IMPORTED_MODULE_0__game__["a" /* default */](1, playerName, gameOver);
    })
  );
});
...
class Game {
  constructor(numPlayers = 3, playerName = 'Player 1', handleGameOver) {
    this.paused = false;
    this.won = false;
  ...
  drawLoop() {
    ...
    ctx.fillRect(0, 0, canvasWidth, __WEBPACK_IMPORTED_MODULE_3__global__["d" /* SCORE_HEIGHT */]);

    ctx.fillStyle = "rgba(180, 180, 180, .3)";
    ctx.fillRect(0, 0, 484, __WEBPACK_IMPORTED_MODULE_3__global__["d" /* SCORE_HEIGHT */]);

    const barSize = Math.ceil((484 * this.lagPortion));
    ctx.fillStyle = this.user.baseColor;
    ctx.fillRect(0, 0, barSize, __WEBPACK_IMPORTED_MODULE_3__global__["g" /* SQUARE_SIZE */]);

    ctx.fillStyle = "white";
    ctx.font = "14px Arial";
    ctx.fillText((this.lagPortion * 100).toFixed(1) + "%", 5, __WEBPACK_IMPORTED_MODULE_3__global__["g" /* SQUARE_SIZE */] - 5);
    ...
    if (this.userPortion > 0.75) this.won = true;
  }
...
class Player {
  ...
  draw(ctx) {
    ...
    ctx.fillText(this.name, this.posX + __WEBPACK_IMPORTED_MODULE_1__global__["g" /* SQUARE_SIZE */] / 2, this.posY - 10);
  }
