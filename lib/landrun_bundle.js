...
/* harmony export (immutable) */ __webpack_exports__["i"] = SQUARE_SIZE;

const SPEED = 5;
/* harmony export (immutable) */ __webpack_exports__["h"] = SPEED;

const FRAME = 24;
/* harmony export (immutable) */ __webpack_exports__["c"] = FRAME;

const SCORE_HEIGHT = 30;
/* harmony export (immutable) */ __webpack_exports__["f"] = SCORE_HEIGHT;

const SHADOW_SIZE = 3;
/* harmony export (immutable) */ __webpack_exports__["g"] = SHADOW_SIZE;

const MIN_SQ = 7;
/* harmony export (immutable) */ __webpack_exports__["e"] = MIN_SQ;

const MAX_SQ = 32;
/* harmony export (immutable) */ __webpack_exports__["d"] = MAX_SQ;

const WIN_PER = 0.5;
/* harmony export (immutable) */ __webpack_exports__["j"] = WIN_PER;
...
const squaresIntersect = (a, b) => {
  if (a < b) {
    return b < a + __WEBPACK_IMPORTED_MODULE_0__global__["i" /* SQUARE_SIZE */];
  } else {
    return a < b + __WEBPACK_IMPORTED_MODULE_0__global__["i" /* SQUARE_SIZE */];
  }
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  newGameBtns.forEach(btn => btn.addEventListener('click', () => {
    ...
    allModals.forEach(modal => { modal.className = 'game-modal'; });
    game = new __WEBPACK_IMPORTED_MODULE_0__game__["a" /* default */](3, playerName, gameOver);
    })
  );
});
...
class Game {
  constructor(numPlayers = 5, playerName = 'Player 1', handleGameOver) {
    this.paused = false;
    this.won = false;
    ...
    this.initializeUser(playerName);
  ...
  initializeCanvas() {
    ...
    this.gameHeight = this.canvasHeight - __WEBPACK_IMPORTED_MODULE_3__global__["f" /* SCORE_HEIGHT */];
  }
  ...
  initializeUser(playerName) {
    ...
    if (playerName) {
      this.user.name = playerName.slice(0,13);
    }
    this.user.cpu = false;
    // debugger;
  ...
  makePlayer() {
    ...
    const playerRow = randomNum(0, __WEBPACK_IMPORTED_MODULE_3__global__["a" /* BOARD_SIZE */]);
    const playerCol = randomNum(0, __WEBPACK_IMPORTED_MODULE_3__global__["a" /* BOARD_SIZE */]);
    const options = {
      posX: playerCol * __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */],
      posY: playerRow * __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */],
  ...
  centerOnPlayer(player, pos) {
    const xOff = Math.floor(player.posX - (this.gameWidth - __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */]) / 2);
    const yOff = Math.floor(player.posY - (this.gameHeight - __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */]) / 2);
    pos[0] = Math.max(
      Math.min(
        xOff, this.grid.size * __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */] + __WEBPACK_IMPORTED_MODULE_3__global__["b" /* BORDER */] * 2 - this.gameWidth
      ), 0);
    pos[1] = Math.max(
      Math.min(
        yOff, this.grid.size * __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */] + __WEBPACK_IMPORTED_MODULE_3__global__["b" /* BORDER */] * 2 - this.gameHeight
      ), 0);
  }
  ...
  update() {
    ...
    for (let i = 0; i <= 1; i++) {
      if (animateTo[i] !== offset[i]) {
        ...
        magnitude = Math.min(__WEBPACK_IMPORTED_MODULE_3__global__["h" /* SPEED */], Math.abs(delta));
  ...
  area(player) {
    const xDest = player.col * __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */];
    const yDest = player.row * __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */];
  ...
  drawGrid() {
    ...
    ctx.fillRect(0, 0, __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */] * __WEBPACK_IMPORTED_MODULE_3__global__["a" /* BOARD_SIZE */], __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */] * __WEBPACK_IMPORTED_MODULE_3__global__["a" /* BOARD_SIZE */]);

    this.drawGridBorder();

    const minRow = Math.max(Math.floor((offset[1] - __WEBPACK_IMPORTED_MODULE_3__global__["b" /* BORDER */]) / __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */]), 0);
    const minCol = Math.max(Math.floor((offset[0] - __WEBPACK_IMPORTED_MODULE_3__global__["b" /* BORDER */]) / __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */]), 0);
    const maxRow = Math.min(
      Math.ceil((offset[1] + gameHeight) / __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */]), grid.size);
    const maxCol = Math.min(
      Math.ceil((offset[0] + gameWidth) / __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */]), grid.size);

    for (let row = minRow; row < maxRow; row++) {
      for (let col = minCol; col < maxCol; col++) {
        const player = this.grid.get(row, col);
        const x = col * __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */];
        const y = row * __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */];
        let baseColor;
        let shadowColor;
        ...
        this.ctx.fillRect(x, y, __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */], __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */]);
      }
    }
  }
  ...
  drawGridBorder() {
    ...
    const gridWidth = __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */] * __WEBPACK_IMPORTED_MODULE_3__global__["a" /* BOARD_SIZE */];
  ...
  drawLoop() {
    ...
    ctx.save();
    ctx.beginPath();
    ctx.translate(-offset[0] + __WEBPACK_IMPORTED_MODULE_3__global__["b" /* BORDER */], -offset[1] + __WEBPACK_IMPORTED_MODULE_3__global__["b" /* BORDER */] + __WEBPACK_IMPORTED_MODULE_3__global__["f" /* SCORE_HEIGHT */]);
    ...
    players.forEach((p) => {
      ...
      if (fr < __WEBPACK_IMPORTED_MODULE_3__global__["c" /* FRAME */])
        p.draw(ctx, fr / __WEBPACK_IMPORTED_MODULE_3__global__["c" /* FRAME */]);
      else
        p.draw(ctx);
    });
    ...
    ctx.fillRect(0, 0, canvasWidth, __WEBPACK_IMPORTED_MODULE_3__global__["f" /* SCORE_HEIGHT */]);

    ctx.fillStyle = "rgba(180, 180, 180, .3)";
    ctx.fillRect(0, 0, 484, __WEBPACK_IMPORTED_MODULE_3__global__["f" /* SCORE_HEIGHT */]);

    const barSize = Math.ceil((484 * this.lagPortion));
    ctx.fillStyle = this.user.baseColor;
    ctx.fillRect(0, 0, barSize, __WEBPACK_IMPORTED_MODULE_3__global__["f" /* SCORE_HEIGHT */]);

    ctx.fillStyle = "#333";
    ctx.font = "14px 'Press Start 2P'";
    ctx.fillText((this.lagPortion * 100).toFixed(1) + "%", 5+2, __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */] + 4)

    ctx.fillStyle = "white";
    ctx.font = "14px 'Press Start 2P'";
    ctx.fillText((this.lagPortion * 100).toFixed(1) + "%", 5, __WEBPACK_IMPORTED_MODULE_3__global__["i" /* SQUARE_SIZE */] + 2);
    ...
    if (this.gameOver) {
      ...
      document.removeEventListener('keydown', this.listener);
      return this.handleGameOver(true, this.user.name);
    } else if (this.paused) {
      ...
      ctx.fillText('paused', 170, 240);

      window.cancelAnimationFrame(this.frame);
    } else {
      this.frame = window.requestAnimationFrame(this.drawLoop);
    }
    console.log(this.userPortion);
    if (this.userPortion > __WEBPACK_IMPORTED_MODULE_3__global__["j" /* WIN_PER */]) this.won = true;
  }
...
class Player {
  ...
  row() {
    ...
    return this.nearestInteger(currentHeading === 2, posY / __WEBPACK_IMPORTED_MODULE_1__global__["i" /* SQUARE_SIZE */]);
  }

  col() {
    const { currentHeading, posX } = this;
    return this.nearestInteger(currentHeading === 1, posX / __WEBPACK_IMPORTED_MODULE_1__global__["i" /* SQUARE_SIZE */]);
  }
  ...
  draw(ctx) {
    ...
    const offSetPosY = this.posY - __WEBPACK_IMPORTED_MODULE_1__global__["g" /* SHADOW_SIZE */];
    ctx.fillStyle = this.shadowColor;
    ctx.fillRect(this.posX, this.posY, __WEBPACK_IMPORTED_MODULE_1__global__["i" /* SQUARE_SIZE */], __WEBPACK_IMPORTED_MODULE_1__global__["i" /* SQUARE_SIZE */]);

    ctx.fillStyle = this.baseColor;
    ctx.fillRect(this.posX, offSetPosY, __WEBPACK_IMPORTED_MODULE_1__global__["i" /* SQUARE_SIZE */], __WEBPACK_IMPORTED_MODULE_1__global__["i" /* SQUARE_SIZE */]);
    ctx.strokeRect(this.posX, offSetPosY, __WEBPACK_IMPORTED_MODULE_1__global__["i" /* SQUARE_SIZE */], __WEBPACK_IMPORTED_MODULE_1__global__["i" /* SQUARE_SIZE */]);

    ctx.fillStyle = this.baseColor;
    ctx.textAlign = "center";
    ctx.fillText(this.name, (this.posX + __WEBPACK_IMPORTED_MODULE_1__global__["i" /* SQUARE_SIZE */] / 2) + 1, this.posY - 9);
    ctx.fillStyle = '#333';
    ctx.textAlign = "center";
    ctx.fillText(this.name, this.posX + __WEBPACK_IMPORTED_MODULE_1__global__["i" /* SQUARE_SIZE */] / 2, this.posY - 10);
  }
  ...
  cpuKeepAlive() {
    ...
    if (this.lastMoveTime === null ||
      (new Date() - this.lastMoveTime) > 500 ) {
        ...
        this.heading = newHeadings[__WEBPACK_IMPORTED_MODULE_2__math_util__["randomNum"](0,3)];
        this.lastMoveTime = new Date();
    }
    const col = this.col();
    const row = this.row();
    if (col < __WEBPACK_IMPORTED_MODULE_1__global__["e" /* MIN_SQ */]) {
      this.heading = 0;
      this.lastMoveTime = new Date();
    }

    if (row < __WEBPACK_IMPORTED_MODULE_1__global__["e" /* MIN_SQ */]) {
      this.heading = 1;
      this.lastMoveTime = new Date();
    }

    if (col > __WEBPACK_IMPORTED_MODULE_1__global__["d" /* MAX_SQ */]) {
      this.heading = 2;
      this.lastMoveTime = new Date();
    }

    if (row > __WEBPACK_IMPORTED_MODULE_1__global__["d" /* MAX_SQ */]) {
      this.heading = 3;
      this.lastMoveTime = new Date();
    }
  }
  ...
  move() {
    ...
    if (this.posX % __WEBPACK_IMPORTED_MODULE_1__global__["i" /* SQUARE_SIZE */] !== 0 || this.posY % __WEBPACK_IMPORTED_MODULE_1__global__["i" /* SQUARE_SIZE */] !== 0) {
    ...
    switch (heading) {
      case 0:
        this.posY -= __WEBPACK_IMPORTED_MODULE_1__global__["h" /* SPEED */];
        break;
      case 1:
        this.posX += __WEBPACK_IMPORTED_MODULE_1__global__["h" /* SPEED */];
        break;
      case 2:
        this.posY += __WEBPACK_IMPORTED_MODULE_1__global__["h" /* SPEED */];
        break;
    ...
    if (this.grid.isOutOfBounds(row, col)) {
      ...
      if (this.grid.get(row, col) === this) { // SAFE!
        ...
        this.posX % __WEBPACK_IMPORTED_MODULE_1__global__["i" /* SQUARE_SIZE */] === 0 &&
        this.posY % __WEBPACK_IMPORTED_MODULE_1__global__["i" /* SQUARE_SIZE */] === 0) {
...
class Tail {
  ...
  drawCorner(ctx, cornerStart, last, current) {
    ...
    fold.moveTo(cornerStart[1] * __WEBPACK_IMPORTED_MODULE_2__global__["i" /* SQUARE_SIZE */], cornerStart[0] * __WEBPACK_IMPORTED_MODULE_2__global__["i" /* SQUARE_SIZE */]);
    fold.lineTo(a[1] * __WEBPACK_IMPORTED_MODULE_2__global__["i" /* SQUARE_SIZE */], a[0] * __WEBPACK_IMPORTED_MODULE_2__global__["i" /* SQUARE_SIZE */]);
    fold.lineTo(b[1] * __WEBPACK_IMPORTED_MODULE_2__global__["i" /* SQUARE_SIZE */], b[0] * __WEBPACK_IMPORTED_MODULE_2__global__["i" /* SQUARE_SIZE */]);
    fold.closePath();
    for (let i = 0; i < 2; i++) {
      ctx.fill(fold);
    }
  ...
  fillTailRect(ctx, start, end) {
    let col = start[1] * __WEBPACK_IMPORTED_MODULE_2__global__["i" /* SQUARE_SIZE */];
    let row = start[0] * __WEBPACK_IMPORTED_MODULE_2__global__["i" /* SQUARE_SIZE */];
    let width = (end[1] - start[1]) * __WEBPACK_IMPORTED_MODULE_2__global__["i" /* SQUARE_SIZE */];
    let height = (end[0] - start[0]) * __WEBPACK_IMPORTED_MODULE_2__global__["i" /* SQUARE_SIZE */];

    if (width === 0) {
      width += __WEBPACK_IMPORTED_MODULE_2__global__["i" /* SQUARE_SIZE */];
    }
    if (height === 0) {
      height += __WEBPACK_IMPORTED_MODULE_2__global__["i" /* SQUARE_SIZE */];
    }
